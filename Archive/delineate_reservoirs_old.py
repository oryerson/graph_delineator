...
def split_at_reservoir_outlet(
    ...
    print(f"    DEBUG {res_id}: outlet=({outlet_lng:.6f}, {outlet_lat:.6f}), "
          f"node={outlet_node}, catchment_area={catchment_poly.area:.8f}")
    ...
    if upstream_poly is None:
        print(f"    DEBUG {res_id}: split returned None")
        return outlet_node

    if upstream_poly.equals(catchment_poly):
        print(f"    DEBUG {res_id}: split returned full catchment (outlet at bottom)")
        return outlet_node

    print(f"    DEBUG {res_id}: split OK — upstream={upstream_poly.area:.8f}, "
          f"remainder={catchment_poly.area - upstream_poly.area:.8f}")
...
def insert_reservoirs(
    ...
    For each reservoir:
    1. Find all graph nodes whose catchment intersects the reservoir
    2. Find the reservoir outlet using flow accumulation raster
    3. If needed, split the catchment at the outlet to create a new node
    4. Merge all intersecting nodes into the outlet node
    5. Flag as is_reservoir=True to protect from consolidation
    ...
    for _, res in reservoirs.iterrows():
        ...
        # Force to string (integers) to match COMID string format and avoid Parquet mixed-type errors
        try:
        ...
        # 2. Find the reservoir outlet using flow accumulation
        ...
        # 3. Determine which catchment contains the outlet
        outlet_node = next(
            (n for n in G.nodes
             if G.nodes[n].get("polygon") is not None
        ...
        print(f"    DEBUG {res_id}: outlet_node={'NONE' if outlet_node is None else outlet_node}, "
              f"intersecting={len(intersecting)} nodes")

        # 4. Split the catchment at the outlet if needed, or use the
        #    most-downstream intersecting node as fallback
        if outlet_node is not None:
            ...
            # Outlet is at an existing node boundary
            target = find_downstream_node(G, intersecting)

        # 5. Merge all other intersecting nodes INTO the target.
        #    IMPORTANT: when the split succeeded (target != outlet_node),
        #    do NOT merge outlet_node back in — it holds the downstream
        #    remainder and merging it would undo the split.
        merged_nodes = []
        for node in list(intersecting):
            if node == target:
                continue
            if node == outlet_node and target != outlet_node:
                continue  # preserve the downstream remainder from the split
            if node not in G:
                continue
            if G.nodes[node].get("is_gauge", False):
                continue
            info = merge_nodes(G, source=node, target=target, reason="reservoir_merge")
            if info:
                merged_nodes.append(info["source"])

        # 6. Flag the merged node so consolidation won't consume it
    
        G.nodes[target]["is_reservoir"] = True
        G.nodes[target]["node_type"] = "reservoir"
        G.nodes[target]["is_gauge"] = False

        # 7. Relabel to reservoir ID if not already
        if target != res_id:
